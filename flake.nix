  ...
  outputs = { self, nixpkgs, flake-utils-plus, ... }@inputs:
    flake-utils-plus.lib.mkFlake {
      ...
      hostDefaults = {
        ...
        specialArgs = { inherit (inputs) self nixos-hardware impermanence; };
      };

      hosts = {
        # Personal laptop, Dell Inspiron 3543 A10
        oldbook.modules =
          [ self.modules.desktops.river self.hostModules.oldbook ];

        # Online VPS, Linode
        histoire.modules = [ self.hostModules.histoire ];
      };

      overlay = import ./pkgs { inherit inputs; };
      overlays =
        flake-utils-plus.lib.exportOverlays { inherit (self) inputs pkgs; };
    };
}
